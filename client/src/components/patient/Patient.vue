<template>
  <div>
    <v-container grid-list-md pa-2 fluid fill-height>
      ...
      <v-layout
        row
        :class="{ wrap: $vuetify.breakpoint.xs }"
        style="background: 0"
      >
        <v-flex
          style="width: 270px"
        ...
        <v-flex :class="{ patientData: $vuetify.breakpoint.smAndUp }" grow>
          <patient-data
            v-if="patient"
            :is="registryComponents[$store.state.activeRegistry.shortName]"
            :componentP="dataComponent"
          ></patient-data>
...
export default {
  components: {
    PatientBar,
    PatientList,
    PatientDataSarcoidosis,
    PatientDataMedicalOncology,
  },
  data() {
    return {
      search: null,
      ...
      registryComponents: {
        sarcoidosis: PatientDataSarcoidosis,
        medical_oncology: PatientDataMedicalOncology,
        heart_failure: PatientDataHeartFailure,
      },
    }
  },
  watch: {
