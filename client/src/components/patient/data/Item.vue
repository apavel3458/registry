                  ...
                  <v-flex xs12 sm4 v-for="(detail, i) in itemDetails" :key="i">
                    ...
                    <template v-else-if="detail.type == 'combobox string'">
                      <v-combobox
                        ...
                        clearable
                        @input.native="
                          editedItem.details[detail.name] =
    ...
    <div class="tableWrapper">
      ...
      >
        <template v-slot:item="props">
          <tr
            @click="
              !expanded.length || expanded[0] != props.item
                ? (expanded = [props.item])
                : (expanded = [])
            "
          >
        ...
        <template v-slot:expanded-item="props">
          <td :colspan="props.headers.length">
            <v-card class="elevation-1 expandedRow">
              <v-card-text>
                <span
                  v-for="(d, i) in itemsAndDetails(props.item)"
                  v-bind:key="i"
                  class="detailParent"
                >
                  <v-chip
                    :outlined="d.type == 'boolean' && !d.value"
                    :class="{ falseBoolean: d.type == 'boolean' && !d.value }"
                  >
                    <span class="detailHeader">{{ d.text }}</span>
                    <span
                      class="detailValue"
                      v-if="
                        props.item.details[d.name] !== true &&
                        !!props.item.details[d.name]
                      "
                    >
                      <span class="detailHeader">:</span>
                      {{ props.item.details[d.name] }}
                    </span>
                  </v-chip>
                </span>
...
export default {
  ...
  computed: {
    ...
    itemDetails() {
      ...
      var item = this.itemTypes.filter(
        (x) => x[this.itemTypeName] == this.editedItem[this.itemTypeName]
      )
  ...
  methods: {
    ...
    itemsAndDetails(item) {
      ...
      let typeFound = this.itemTypes.find(
        (t) => t[this.itemTypeName] == item[this.itemTypeName]
      )
    ...
    editItem(item) {
      ...
      const itemTypeFound = this.itemTypes.find(
        (t) => t[this.itemTypeName] == item[this.itemTypeName]
      )

      if (itemTypeFound)
        this.editedItem.details = _.pick(
          this.editedItem.details,
          itemTypeFound.details.map((x) => {
            return x.name
          })
        )
    ...
    async deleteItem(item) {
      ...
      if (
        confirm(
          'Are you sure you want to delete this ' + this.itemType + ' item?'
        )
      ) {
        await RegistryPatientDataService.itemDelete(item, this.itemType)
          .then(() => {
            //this.items.splice(index, 1);
            this.$store.commit('ACTIVE_PATIENT_PROPERTY_DELETE', {
              prop: this.itemType,
              index: index,
            })
      ...
      if (this.editedIndex > -1) {
        ...
        await RegistryPatientDataService.itemPut(this.editedItem, this.itemType)
          .then((reply) => {
            ...
            this.$store.commit('ACTIVE_PATIENT_PROPERTY_UPDATE', {
              prop: this.itemType,
              val: reply,
        ...
        //new item
        await RegistryPatientDataService.itemPost(
          this.editedItem,
          this.itemType
        )
          .then((reply) => {
            //this.items.push(reply);
            this.$store.commit('ACTIVE_PATIENT_PROPERTY_ADD', {
              prop: this.itemType,
              val: reply,
            })
    ...
    getItemSchema(item) {
      if (item.schema) {
        ...
        let result = this.itemTypes.find(
          (t) => t[this.itemTypeName] == item[this.itemTypeName]
        )
        if (!result) return []
        else {
    ...
    getVisibleDetail(item) {
      ...
      if (visibleDetailName) {
        let visibleDetail = itemSchema.details.find(
          (d) => d.name == visibleDetailName
        )
        if (visibleDetail && visibleDetail.type == 'boolean')
          return item.details[visibleDetailName] ? visibleDetail.text : blank
        //if boolean
        else return item.details[visibleDetailName]
      } else {
        return '--'
      }
    },
