<template>
  <item-block :typesJson="typesJson" itemType="diagnosis">
    <template v-slot:displayData="props">
      <v-list-item>
        <v-list-item-content class="itemHeading">
          Start Date
        </v-list-item-content>
      ...
      <v-list-item>
        ...
        <v-list-item-content class="itemContent">
          {{ props.item.dateEnd ? props.item.dateEnd : '--' }}
        </v-list-item-content>
      </v-list-item>
      <v-list-item>
        <v-list-item-content class="itemHeading">Type</v-list-item-content>
        <v-list-item-content class="itemContent">
          {{ props.item.details.type ? props.item.details.type : '--' }}
        </v-list-item-content>
      </v-list-item>
      <v-list-item>
        <v-list-item-content class="itemHeading">NYHA</v-list-item-content>
        <v-list-item-content class="itemContent">
          {{ props.item.details.NYHA ? props.item.details.NYHA : '--' }}
        </v-list-item-content>
      </v-list-item>
    </template>
    <template v-slot:dataEdit="{ editedItem }">
      ...
      <v-flex xs12>
        <v-select
          ...
          required
          :rules="[
            () => !!editedItem.diagnosisName || 'This field is required',
          ]"
          style="width: 50%"
        ></v-select>
      </v-flex>
